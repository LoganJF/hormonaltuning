"""processingpipeline.py functions to aid in analyses done for (Fickling et al., 2025)
...
TODO:cluster.py should have the sklearn clustering stuff instead
"""
...
def feature_extraction_preprocessing_pipeline(date, condition, paths=None, spike_df=None,
                                              filler_value=-100,
    ...
    """Preprocesses spike data and long IC data, extracting features for further analysis.

    :param date: str, Date of the experiment.
    :param condition: str, Experimental condition (e.g., 'control', 'drug').
    :param paths: list of str, Optional, alternative way of loading paths to data files.
    :param spike_df: pd.DataFrame, alternative way to pass in spike data
    :param filler_value: int, Value used to fill missing phase data when long IC is absent.
    :param segment_duration_seconds: int, Duration (in seconds) of each data segment for feature extraction.
    :param thres_max_CP_long_IC: int, Maximum threshold for phase calculation in long IC data.
    :param thres_max_time_from_nearest_phase: int, Threshold for the maximum time from the nearest phase.
    :param n_sec_cp_combine: int, Threshold to combine close bursts in long IC data.
    :param long_IC_burst_dur: int, Minimum burst duration (in seconds) for long IC data.
    :param neurons_keep: list, List of neurons to keep in the analysis.
    :param phase_bins: int, Number of bins for phase space division.
    :param verbose: bool, Whether to print verbose output.

    :return: pd.DataFrame, DataFrame containing extracted features for each data segment.
    """
    # Check if passed in a dataframe of spike data
    if spike_df is None: # If not, load spike data
        spike_df = load_spikes(date, paths=paths)


    # assure it's only 900s of data, remove irrelevant neurons, and remove first minute of incubation
    spike_df = spike_df.loc[spike_df.condition == condition]
    ...
    ids = ['{} {} segment {}'.format(date, condition, i) for i in np.arange(len(segment_features_df))]
...
def extract_features_from_segment(segment_df,
                                  ...
                                  neurons_keep):
    """Extracts various spike timing and phase features from a given segment of data.

    :param segment_df: pd.DataFrame, DataFrame containing the spike data for the segment.
    :param segment_start: float, Start time of the segment.
    :param long_IC_editted: pd.DataFrame, DataFrame containing the processed long IC data.
    :param segment_duration_seconds: int, Duration (in seconds) of the segment.
    :param neurons_keep: list, List of neurons to keep in the analysis.

    :return: pd.DataFrame, DataFrame containing the features for the segment.
    """
    df = segment_df
...
def concat_to_single_feature_dataframe_per_segment(isi_deciles,
                                                   ...
                                                   neurons_keep):
    """Concatenates multiple features (such as ISI deciles, phase deciles, firing rate, ratios, and burstiness)
    into a single DataFrame for each neuron in the segment. This provides a comprehensive feature set for analysis.

    :param isi_deciles: pd.DataFrame, Contains ISI deciles for each neuron.
    :param phase_deciles: pd.DataFrame, Contains phase deciles for each neuron.
    :param firing_rate: pd.DataFrame, Contains mean firing rates for each neuron.
    :param ratio_second_order_first_order_isi: pd.DataFrame, Contains the ratio of second-order to first-order ISIs for each neuron.
    :param ratio_largest_second_largest_isi: pd.DataFrame, Contains the ratio of the largest to the second-largest ISI for each neuron.
    :param burstiness: pd.DataFrame, Contains the burstiness measure for each neuron.
    :param number_long_IC_in_segment: int, The number of long IC bursts in the segment.
    :param neurons_keep: list, List of neurons to include in the feature concatenation.

    :return: pd.DataFrame, A DataFrame containing all features concatenated per neuron, including additional features and the number of long IC bursts.
    """
    cols = []
    _data = []
